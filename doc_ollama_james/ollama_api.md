...
image OCR, and image descriptions. Its shared API client is the `ollama_api`
class in `lib/wrapp_ollama.py`. James flows invoke it through `runner.py` and
`cli_ollama.py`; Cowork uses its configuration in `lib/wrapp_agent.py`, which
sends tool-calling requests to `/api/chat`. OCR uses `/api/generate`.
...
- `assistant/tasks/task_ocr.json` currently uses `deepseek-ocr:3b`.
- `assistant/tasks/task_translate.json` currently uses `translategemma:12b`.

Shared defaults are in `lib/ollama.json`; task options and CLI overrides refine
them. Run commands from the project root, for example
`python cli_ollama.py --type task_ocr.json --in camera.png --out ocr.txt`.
