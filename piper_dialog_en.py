"""Přehrává dialog ze zvoleného textového souboru hlasy Piperu."""
...
DIALOG_FILE = PROJECT_DIR / "dialog_en.txt"
...
# Nižší length scale = rychlejší řeč, vyšší = pomalejší řeč.
ALAN_LENGTH_SCALE = 0.9
CORI_LENGTH_SCALE = 1.1

SHORT_PAUSE_SECONDS = 1.5
LONG_PAUSE_SECONDS = 3


VOICE_MODELS = {
    "alan": ASSETS_DIR / "en_GB-alan-medium.onnx",
    "cori": ASSETS_DIR / "en_GB-cori-medium.onnx",
}
SPEAKERS = {
    "voice1": VOICE1,
    "voice2": VOICE2,
...
VOICE_LENGTH_SCALES = {
    "alan": ALAN_LENGTH_SCALE,
    "cori": CORI_LENGTH_SCALE,
}
...
def read_dialog(path: Path) -> list[tuple[str, str | float]]:
    ...
    if not any(event_type != "pause" for event_type, _ in events):
        raise ValueError("dialog_en.txt neobsahuje žádnou repliku.")
    return events
...
def run_dialog(dialog_path: Path, output_dir: Path) -> int:
    try:
    ...
    try:
        ...
        for voice_name in set(SPEAKERS.values()):
            ...
            synthesis_configs[voice_name] = SynthesisConfig(
                length_scale=VOICE_LENGTH_SCALES[voice_name]
            )
        ...
        for event_type, value in dialog:
            ...
            with wave.open(str(wav_path), "wb") as wav_file:
                ...
                voices[voice_name].synthesize_wav(
                    text, wav_file, syn_config=synthesis_configs[voice_name]
                )
            if first_speech:
        ...
        if MP3:
            ...
            output_path = output_dir / f"piper_{dialog_path.stem}_{timestamp}.mp3"
...
def main() -> int:
    return run_dialog(parse_arguments(), PROJECT_DIR)
