...
Schema je v `assistant/tools/tool_schema.json` a implementace v
`assistant/tools/tool_files.py`.
...
| `run_command(command)` | spuštění shellového příkazu | vždy vyžádá potvrzení `[y/N]` |
...
- `tool_files.py` spoléhá na aktuální pracovní adresář. Model by teoreticky
  mohl předat absolutní cestu mimo projekt; systémový prompt to zakazuje, ale
  kód to zatím nevynucuje.
- Zápis souboru nemá vlastní potvrzení.
- Neexistuje strojově čitelný záznam jednoho běhu: které nástroje se volaly,
  jaké soubory vznikly a jak dopadly příkazy.
...
| Shell | explicitně přes `/tool` | vždy s viditelným potvrzením |
...
- Implementace souborových toolů se změní tak, aby místo globálního cwd
  přijímaly nebo uzavíraly `ProjectToolScope`.
- Ze schema je vhodné časem vytvořit registry s metadaty policy. Samotné JSON
  schema totiž neříká, zda je volání čtecí, zapisující nebo nebezpečné.
...
- [ ] Implementovat `ProjectToolScope(root: Path)`.
- [ ] Odmítnout absolutní cesty a cesty, které po resolve opustí root projektu.
- [ ] Přidat policy wrapper nad každým toolem.
- [ ] Přidat zápis `AgentRun` do paměti a výpis finálního reportu.
- [ ] Přidat testy pro `..`, absolutní cesty, neznámé argumenty a odmítnutý
      příkaz.
- [ ] Přidat testy pro tool loop s mocknutou Ollama odpovědí, včetně více tool
...
- [ ] Přesunout agentní engine do `lib/wrapp_agent.py`.
- [ ] Nechat `cli_agent.py` jako kompatibilní CLI adaptér.
- [ ] Zachovat existující argumenty: `--model`, `--prompt`, `--timeout`,
      `--max-steps`, `--verbose`.
- [ ] Doplnit `--policy observe|draft|code` a volitelný dočasný projektový
      override validovaný uvnitř rootu.
- [ ] Přidat jednotkové testy knihovny bez závislosti na běžící Ollamě.
...
- [ ] Nahradit `show_mock(config, "cowork")` funkcí `cowork_menu(config)`.
- [ ] Vytvořit `cowork_code_menu(config)` s položkami z návrhu menu.
- [ ] Přidat volbu modelu, projektu a policy pro aktuální Cowork session.
- [ ] Spustit shared `AgentEngine` a renderovat události přes `Terminal`.
- [ ] Po ukončení relace vykreslit `AgentRun` report a nabídnout návrat do
      Cowork menu.
- [ ] Zajistit, že přepnutí projektu v Coworku je pouze session-local, stejně
